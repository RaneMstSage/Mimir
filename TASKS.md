...
- [~] App installs and runs (via Downloads → My Files; Termux can't call the installer on Android 14+). DevTools attach result still awaited. Verify same-uid DevTools connect, Origin strip, frontend attach, Elements picker. Fix what breaks. (Runtime debugging without logcat: add a temporary in-app status text of bridge/attach errors.) Commit.
